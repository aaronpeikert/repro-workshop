...
The idea for the data stems from the [tidytuesday initiative](https://github.com/rfordatascience/tidytuesday/blob/master/data/2021/2021-03-16/readme.md); the data was scraped for [kaggle](https://thislinkdoesnotwork.error) from [steamcharts.com](https://steamcharts.com/).
...
We have `r length(unique(games$gamename))` different games in the dataset.
Each game was in the dataset for on average of `r games %>% group_by(gamename) %>% count() %>% pull(n) %>% mean() %>% round(2)` months.

```{r summary-avg}
summary(games$avg) %>% # pander makes markdown tables
  pander(caption = "Summary of avarage simultaneous gamers.")
```
...
This is also an estimate for the internal consistency (aka Cohen's α).
...
If we instead correlate average and peak simultaneous gamers for each game separate and then average these correlations, we get a within game correlation of `r round(within_cor, 4)`. Remember this is a somewhat crude way to sidestep the simpson paradox [@simpson_interpretation_1951] and a real mixed effect model would be state of the art [@balakrishnan_generalized_2014].
