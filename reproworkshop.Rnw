...
\begin{frame}[c, fragile]{Download this GitHub repo}
  ...
  <<gh-setup, include=FALSE>>=
  options(repro.github.token = TRUE)
  options(repro.github.ssh = TRUE)
  @
  
  <<gh, eval=FALSE>>=
  if(repro::has_github_token()) {
    options(usethis.protocol = "https")
  } else if(repro::has_github_ssh()) {
    options(usethis.protocol = "ssh")
  }
  
  usethis::create_from_github(
    "aaronpeikert/repro-workshop",
    #creates new folder 'repro-workshop'
    destdir = "path/to/some/folder",
    #if you are brave, try fork = TRUE
    fork = FALSE)
...
\begin{frame}[fragile]{Code Chunk Options \onslide<3->{(5 min)}}
...
\begin{frame}{Reproduce \lstinline[basicstyle=\ttfamily]{games.Rmd} \onslide<2->{(5 min)}}
